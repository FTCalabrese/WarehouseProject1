        ...
        if(xhr.status == 200 && warehouseID === undefined)
        {
            ...
            for(item of inventory)
            {
                ...
                if(item._ownerId == owner.toString())
                {
                    ...
                    li.innerHTML = `<button class="nav-link" onclick="customEvent('${item.warehousename}')" id="profile-tab" data-bs-toggle="tab" data-bs-target="#w${warehouseCount}" type="button" role="tab" aria-controls="profile" aria-selected="false" style="color: black">${item.warehousename}</button>`;
...
//delete the table when tabChange event is triggered
window.addEventListener('tabChange', ()=>{
    const data = document.getElementById('database-table');
    data.remove();
})

//triggers tabChange event, and runs getFilteredInventory()
function customEvent(warehouseName){
    const ev = new Event('tabChange');
    window.dispatchEvent(ev);

    getFilteredInventory(warehouseName);
}
