        ...
        if(xhr.status == 200 && warehouseID === undefined)
        {
            ...
            for(item of inventory)
            {
                ...
                if(item._ownerId == owner.toString())
                {
                    ...
                    for(let i = 0; i < item.items.length; i++) populateRow(item, i, warehouseCount, container);
                }
            }
            ...
            const tab = document.createElement('div');
            li.classList = "nav-item";
            li.innerHTML = `<a href='/forms/warehouse?company=${qs.get('company')}'><button class="nav-link" id="new-tab" data-bs-toggle="tab" data-bs-target="#" type="button" role="tab" aria-controls="profile" aria-selected="false" style="color: black"><b> + New</b></button></a>`;
            tab.classList = "tab-pane fade";
            tab.id = ``; tab.innerHTML = ``;
            navbar.append(li); navpage.append(tab);
        }
        else { container.innerText = 'Warehouse is empty'; }
        
    }
    xhr.send();
}
window.addEventListener('DOMContentLoaded', ()=>{ getInventory();});
...
function filterTable(warehouseName){
    ...
    if(warehouseName == '') getFilteredInventory(undefined);
...
function getFilteredInventory(warehouseFilter = undefined)
{
    ...
    xhr.onload = function ()
    {
        ...
        if(xhr.status == 200)
        {
            ...
            for(item of inventory)
            {
                //if there is no filter, print every owned warehouse
                if(item._ownerId == owner.toString() && warehouseFilter == undefined)
                {
                    warehouseCount++;
                ...
                //if there is a filter, print the specified warehouse
                else if(item._ownerId == owner.toString() && item.warehousename == warehouseFilter)
                {
                    warehouseCount++;
...
function populateRow(item, i, warehouseCount, container)
{
    ...
    <td><button type="button" id="edit${warehouseCount},${i}" style='width:50px' class='btn btn-warning shadowed'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-brush" viewBox="0 0 16 16">
    ...
    <td><button type="button" id="${warehouseCount},${i}" style='width:50px' class='btn btn-danger shadowed'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
